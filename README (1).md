...
Input: [0 0] (Moist soil, rain expected) -> Keep irrigation OFF
Input: [0 1] (Moist soil, no rain expected) -> Keep irrigation OFF
Input: [1 0] (Dry soil, rain expected) -> Keep irrigation OFF
Input: [1 1] (Dry soil, no rain expected) -> Turn ON irrigation
```

(Exact weight/bias values may vary slightly depending on training, but decisions will always be correct.)

## Key Concept

This demonstrates that a **single-layer perceptron** can learn simple **linearly separable logic** (like AND) and apply it to a practical agricultural automation scenario — deciding when to irrigate a crop field.
